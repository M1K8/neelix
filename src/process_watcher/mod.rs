...
pub async fn process_watcher(expected_processes: HashSet<String>, chan : tokio::sync::mpsc::Sender<Process>) {
    use sysinfo::{System};
    ...
    let seen_this_cycle_clone = seen_this_cycle.clone();
        for p in seen_this_cycle_clone {
            if !processes.values().any(|proc| proc.name().to_str().unwrap() == p) && is_still_alive.contains(&p) {
